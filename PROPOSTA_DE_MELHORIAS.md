...
- **16 de julho de 2026 — primeira fase de U05 implementada e validada no bundle Android:** tarefas do Perfil, filtros, exclusão, detalhes e labels de acessibilidade passaram a usar o idioma atual. Tipo, rótulos padrão e repetição são resolvidos por chaves semânticas na renderização, portanto uma tarefa criada em inglês aparece em português após a troca de idioma. A suíte passou a ter 26 casos; a remoção dos campos traduzidos redundantes e a auditoria das demais telas continuam pendentes.
...
| U05 — Primeira fase implementada | **Tarefas do Perfil e seus detalhes deixaram de ter textos fixos em inglês.** [ProfileTasksModal.js](./components/ProfileTasksModal.js), [ProfileSwipeTaskCard.js](./components/ProfileSwipeTaskCard.js) e [ProfileTaskDetailModal.js](./components/ProfileTaskDetailModal.js) usam textos e labels de acessibilidade centralizados. Tipo, rótulos padrão e repetição são traduzidos pela chave semântica atual, mesmo quando `tagLabel`/`typeLabel` antigos guardam outro idioma. | A troca de idioma agora atualiza títulos, filtros, ações, detalhes e os rótulos conhecidos sem exigir recriar a tarefa. Tags personalizadas continuam exibindo o texto criado pelo usuário. | Pequeno–médio entregue; restante médio / **Alta** | Parar de persistir `tagLabel` e `typeLabel` redundantes após uma migração compatível e auditar os textos fixos restantes fora deste fluxo. A regressão de tradução elevou T04 para 26 casos. |
...
16. **U05 — primeira fase de internacionalização entregue:** tarefas do Perfil, filtros, ações, detalhes, tipos, rótulos padrão, repetições e acessibilidade acompanham o idioma atual; a persistência redundante e as demais telas continuam para uma passagem posterior.
