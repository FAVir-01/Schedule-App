...
- **Próximo bloco recomendado:** tratar B16; a primeira abertura do calendário em build release, a responsividade de B13 e as validações residuais de B14 continuam pendentes.
...
| T04 — Primeira fase ampliada em B14 | **O projeto possui uma suíte mínima de regras de domínio.** [test-domain-rules.js](./scripts/test-domain-rules.js) usa os helpers reais para cobrir calendário estrito, recorrência, quantum e agora validação de tipo, bytes, dimensões, extensão e mensagens de imagem. O harness isola o runtime nativo e os 11 casos passaram com `npm run test:domain`. | Protege regras onde os bugs se concentraram sem introduzir uma suíte ampla prematuramente. Ainda não cobre UI, perfil, notificações nem a cópia nativa de arquivos. | Pequeno entregue; expansão gradual / **Alta** | Adicionar casos junto de cada bug corrigido. Para imagens, testar o serviço com mock do filesystem e manter os cenários reais de Android/iOS antes do release. |
