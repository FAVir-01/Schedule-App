...
- **Próximo bloco recomendado:** validar a primeira abertura do calendário em build release e tratar B14; responsividade por largura e navegação temporal de B13 continuam em um bloco próprio.
...
| T04 — Primeira fase criada na Sessão 4 | **O projeto passou a ter uma suíte mínima de regras de domínio.** [test-domain-rules.js](./scripts/test-domain-rules.js) usa os helpers reais para cobrir calendário estrito, intervalo de datas, término inclusivo, recorrência semanal, meta quantum e política de preservação/reinício de progresso. O comando está em [package.json](./package.json) como `npm run test:domain`. | Cria proteção inicial justamente nas regras onde os bugs se concentraram, sem introduzir uma suíte ampla prematuramente. Ainda não cobre UI, perfil, persistência ou notificações. | Pequeno entregue; expansão gradual / **Alta** | Executar em ambiente com Node — indisponível nesta sessão — e corrigir qualquer incompatibilidade antes do release. Depois adicionar casos junto de cada bug corrigido, começando por perfil e persistência. |
