...
- **15 de julho de 2026 — B02 mitigado no código e coberto por fixtures:** o perfil e o gráfico reutilizam agendas compiladas por tarefa, evitam listas intermediárias por dia e limitam sequências a uma janela explícita de 730 dias sem alterar o total histórico. As fixtures de 50, 500 e 2.000 tarefas passaram; a fluidez real ainda precisa ser validada em aparelho.
- **Próximo bloco recomendado:** reproduzir B04 com medição/observabilidade mínima de T06; a primeira abertura do calendário em build release, a responsividade de B13 e as validações residuais de B14 continuam pendentes.
...
| B02 — Mitigado no código; validar em aparelho | **O cálculo do perfil deixou de percorrer um intervalo histórico ilimitado com filtros por dia.** [profileStatsUtils.js](./utils/profileStatsUtils.js) preserva o total histórico, limita sequências a 730 dias, compila a agenda de cada tarefa uma vez e interrompe a avaliação diária na primeira pendência. [PerformanceChart.js](./components/PerformanceChart.js) reutiliza o mesmo gerador sem criar listas filtradas por data. | Remove o caminho de datas antigas/corrompidas para intervalos gigantes e reduz alocações e normalizações repetidas com muitas tarefas. A montagem continua proporcional à janela e ao número de tarefas no pior caso, portanto a fluidez em aparelhos modestos ainda precisa ser medida. | Médio entregue / **Alta** | Fixtures com 0, 50, 500 e 2.000 tarefas e data histórica passaram em T04; a interface informa quando a sequência usa a janela de dois anos. Validar abertura, filtros, linha, barras e rolagem em build release, registrando JS/UI FPS, duração e memória. Depende de B04/T06 para fechar. |
...
| T04 — Primeira fase ampliada até B02/B16 | **O projeto possui uma suíte mínima de regras de domínio.** [test-domain-rules.js](./scripts/test-domain-rules.js) usa os helpers reais para cobrir calendário estrito, recorrência, quantum, histórico, imagens, celebração/háptico e estatísticas do perfil. O harness isola o runtime nativo e os 20 casos passaram com `npm run test:domain`, incluindo fixtures de 50, 500 e 2.000 tarefas. | Protege regras onde os bugs se concentraram sem introduzir uma suíte ampla prematuramente. Ainda não cobre UI, notificações nativas nem a cópia real de arquivos. | Pequeno entregue; expansão gradual / **Alta** | Adicionar casos junto de cada bug corrigido. Manter cenários reais de Android/iOS e medições de performance antes do release. |
...
10. **B02 — mitigado no código e coberto por fixtures:** agenda compilada por tarefa, janela segura de 730 dias para sequências e gerador compartilhado pelo gráfico; validação de fluidez em aparelho continua pendente.
