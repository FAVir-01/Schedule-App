...
| T04 — Primeira fase ampliada até U05 | **O projeto possui uma suíte mínima de regras de domínio.** [test-domain-rules.js](./scripts/test-domain-rules.js) usa os helpers reais para cobrir calendário estrito, recorrência, quantum, histórico, imagens, celebração/háptico, estatísticas do perfil, observabilidade, retry do Error Boundary, conteúdo privado de notificações e tradução por chave semântica. O harness isola o runtime nativo e os 27 casos passaram com `npm run test:domain`, incluindo fixtures de 0, 50, 500 e 2.000 tarefas. | Protege regras onde os bugs se concentraram sem introduzir uma suíte ampla prematuramente. Ainda não cobre renderização real da tela de erro, disparos nativos nem a cópia real de arquivos. | Pequeno entregue; expansão gradual / **Alta** | Adicionar casos junto de cada bug corrigido. Manter cenários reais de Android/iOS e medições de performance antes do release. |
