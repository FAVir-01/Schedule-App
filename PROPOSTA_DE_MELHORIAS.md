...
- **Próximo bloco recomendado:** B05, B06 e B07, referentes ao ciclo de vida dos lembretes.
...
| B05 — Confirmado | **Lembretes de tarefas recorrentes são agendados somente uma vez.** O código encontra a próxima ocorrência e cria uma notificação não recorrente em [App.js](./App.js#L1817). Após disparar, o `notificationId` continua armazenado e não há listener para agendar a ocorrência seguinte. | O usuário recebe o primeiro lembrete e deixa de receber os próximos silenciosamente. | Médio / **Alta** | Criar ciclo explícito de reconciliação: consultar notificações ainda agendadas, persistir próxima ocorrência e reagendar após disparo, abertura ou retorno ao app. Depende de T03. |
| B06 — Confirmado | **Exclusões deixam notificações órfãs ou cancelam lembretes indevidamente.** A exclusão pelo card não cancela a notificação em [App.js](./App.js#L2110). Na exclusão em massa, o lembrete é cancelado antes de verificar `profileLocked`; a tarefa bloqueada permanece, mas perde seu lembrete em [App.js](./App.js#L552). | Lembrete de tarefa apagada continua tocando; tarefa bloqueada deixa de notificar. | Pequeno–médio / **Alta** | Centralizar exclusão em um único serviço; ignorar bloqueadas antes de cancelar; limpar ou reconciliar `notificationId`. Depende de B05/T03. |
| B07 — Confirmado | **Configuração de lembrete pode ser salva sem funcionar.** Sem horário especificado, o agendamento retorna `null` em [App.js](./App.js#L1755), mas o formulário aceita a opção. A permissão é solicitada ao abrir o painel em [AddHabitSheet.js](./components/AddHabitSheet.js#L680), e falhas chamadas com `void` podem virar rejeições não tratadas. | O usuário acredita que ativou um lembrete inexistente; solicitações de permissão acontecem fora de contexto. | Médio / **Alta** | Exigir horário, solicitar permissão somente ao confirmar, mostrar resultado e tratar exceções. Depende de B05 e U04. |
...
2. **Sessão 2:** B05, B06 e B07 — bloco coeso de lembretes.
