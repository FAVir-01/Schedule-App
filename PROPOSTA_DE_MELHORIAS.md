...
- **Próximo bloco recomendado:** validar a Sessão 4 no aparelho e tratar B14; os pontos restantes de B13 devem continuar em um bloco próprio de responsividade e navegação temporal.
...
| B13 — Performance inicial implementada; responsividade ainda pendente | **A primeira entrada no calendário fazia trabalho síncrono desnecessário.** [App.js](./App.js) calculava status e humor para aproximadamente 25 meses, cruzando dias e tarefas antes de exibir a aba. Agora calcula somente o mês visível e seus vizinhos e atualiza o conjunto durante a rolagem. Permanecem confirmados o retry vazio de `onScrollToIndexFailed`, o tamanho estático em [layout.js](./constants/layout.js) e a faixa semanal ancorada em “hoje”. | A redução ataca uma causa concreta da leve travada ao entrar; o ganho depende do volume de tarefas e precisa ser medido no aparelho. Os problemas restantes ainda podem abrir na posição errada ou desalinhá-la em mudanças de tamanho. | Pequeno entregue; restante médio / **Média** | Validar primeira entrada e rolagem com 50, 500 e 2.000 tarefas. Em bloco posterior, implementar retry real, layout pela largura atual e recentralização no dia selecionado; testar tablets, split view e font scaling. |
